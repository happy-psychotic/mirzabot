...
class ManagePanel
{
    ...
    function DataUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $UsernameDataRaw = get_clinetsalireza($username, $Get_Data_Panel['name_panel']);
            if (!is_array($UsernameDataRaw) || empty($UsernameDataRaw)) {
                $Output = array(
                    'status' => 'Unsuccessful',
                    'msg' => "user not found"
                );
            } else {
                $clientConfig = isset($UsernameDataRaw[0]) && is_array($UsernameDataRaw[0]) ? $UsernameDataRaw[0] : [];
                $clientStats = isset($UsernameDataRaw[1]) && is_array($UsernameDataRaw[1]) ? $UsernameDataRaw[1] : [];
                $clientId = $clientStats['id'] ?? ($clientConfig['id'] ?? null);

                if (empty($clientId)) {
                    $errorMsg = $clientStats['msg'] ?? ($clientConfig['msg'] ?? "user not found");
                    $Output = array(
                        'status' => 'Unsuccessful',
                        'msg' => $errorMsg
                    );
                } else {
                    $enable = $clientConfig['enable'] ?? ($clientStats['enable'] ?? true);
                    $subId = $clientConfig['subId'] ?? '';
                    $expiryTime = $clientStats['expiryTime'] ?? ($clientConfig['expiryTime'] ?? 0);
                    $expire = intval($expiryTime / 1000);
                    $totalTraffic = isset($clientStats['total']) ? intval($clientStats['total']) : intval($clientConfig['totalGB'] ?? 0);
                    $upTraffic = isset($clientStats['up']) ? intval($clientStats['up']) : 0;
                    $downTraffic = isset($clientStats['down']) ? intval($clientStats['down']) : 0;
                    ...
                    if ($totalTraffic != 0 && ($totalTraffic - ($upTraffic + $downTraffic)) <= 0) {
                        $status = "limited";
                    }
                    ...
                    $subscriptionUrl = rtrim($Get_Data_Panel['linksubx'], '/');
                    if ($subId !== '') {
                    ...
                    $Output = array(
                        'status' => $status,
                        'username' => $clientConfig['email'] ?? ($clientStats['email'] ?? $username),
