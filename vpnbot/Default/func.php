...
function resellerDirectLinkVariants(string $text): array
{
    $trimmed = trim($text);
    if ($trimmed === '') {
        return [];
    ...
    $variants = [$trimmed];
    $normalizedLinks = normalizeConfigLinks([$trimmed]);
    if (is_array($normalizedLinks)) {
        foreach ($normalizedLinks as $normalizedLink) {
            if (is_string($normalizedLink) && trim($normalizedLink) !== '') {
                $variants[] = trim($normalizedLink);
            }
        }
    }
...
function resellerFindOwnedInvoiceByDirectLink(int $userId, string $botToken, string $text)
{
    global $pdo, $ManagePanel;

    $linkVariants = resellerDirectLinkVariants($text);
    if (empty($linkVariants)) {
        return false;
    }

    $directLinkUsername = '';
    $trimmedText = $linkVariants[0];
    ...
    if ($directLinkUsername !== '') {
        ...
        if (!empty($cfgOwnRow['id_invoice'])) {
            return $cfgOwnRow['id_invoice'];
        }
    }

    $stmtInvoices = $pdo->prepare("SELECT * FROM invoice WHERE id_user = :uid AND bottype = :bottype ORDER BY id_invoice DESC LIMIT 100");
    $stmtInvoices->execute([
        ':uid' => $userId,
        ':bottype' => $botToken,
    ]);
    $candidateInvoices = $stmtInvoices->fetchAll(PDO::FETCH_ASSOC);
    foreach ($candidateInvoices as $candidateInvoice) {
        if (empty($candidateInvoice['username']) || empty($candidateInvoice['Service_location'])) {
            continue;
        }

        $panel = select("marzban_panel", "*", "name_panel", $candidateInvoice['Service_location'], "select");
        if (!$panel) {
            continue;
        }
        ...
        try {
            $dataUserOut = $ManagePanel->DataUser(
                $candidateInvoice['username'],
                $panel['name_panel'],
                $panel['type'],
                "select",
                $candidateInvoice['id_user'],
                $candidateInvoice['code_server']
            );
        } catch (Throwable $e) {
            continue;
        }
        ...
        $candidateLinks = resellerAvailableConfigLinks($panel, $dataUserOut);
        foreach ($candidateLinks as $candidateLink) {
            foreach (resellerDirectLinkVariants((string)$candidateLink) as $candidateVariant) {
                if (in_array($candidateVariant, $linkVariants, true)) {
                    return $candidateInvoice['id_invoice'];
                }
            }
        }
    }
